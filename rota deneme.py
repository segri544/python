...
class DroneDataWindow(QWidget):
    ...
    def create_map(self):
        ...
        map_view = self.map_widget.addViewBox()
        map_view.setAspectLocked()
        map_img = pg.ImageItem(np.zeros((1000, 1000)))
        map_view.addItem(map_img)

        # Add location data to the map
        for location in self.location_data:
            marker = pg.ScatterPlotItem(x=location[0], y=location[1], size=10, pen=pg.mkPen(None), brush=pg.mkBrush(255, 0, 0, 120))
            map_view.addItem(marker)
